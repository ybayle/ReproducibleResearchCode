...
def experiments_2_3(train_file):
    utils.print_success("Ghosal Expe 2 & 3 (approx. 30min)")
    train_filenames, train_features, train_groundtruths = read_file(train_file)
    clf = linear_model.RANSACRegressor(random_state=RANDOM_STATE)
    clf.fit(train_features, train_groundtruths)
    test_filenames, test_features, test_groundtruths = read_files("features/database2/")
